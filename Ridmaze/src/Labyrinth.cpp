...
auto Labyrinth::haveAWinner() const -> bool {
  return player && labyrinth[currentLevel].getCell(
                       player->getRow(), player->getCol()) == EXIT_CHAR;
}
...
void Labyrinth::addBlock(Orientation orientation, int startRow, int startCol,
                         int length) {
  ...
  for (int row = startRow, col = startCol;
       posOK(row, col) && emptyPos(row, col) && length > 0;
       row += incRow, col += incCol, --length) {
...
auto Labyrinth::validMoves(int row, int col) const -> std::vector<Directions> {
  ...
  if (canStepOn(row - 1, col))
    moves.push_back(Directions::UP);
  if (canStepOn(row + 1, col))
    moves.push_back(Directions::DOWN);
  if (canStepOn(row, col - 1))
    moves.push_back(Directions::LEFT);
  if (canStepOn(row, col + 1))
    moves.push_back(Directions::RIGHT);
  return moves;
}

auto Labyrinth::posOK(int row, int col) const -> bool {
  return row >= 0 && row < labyrinth[currentLevel].getRows() && col >= 0 &&
         col < labyrinth[currentLevel].getCols(row);
}
...
auto Labyrinth::dir2Pos(int row, int col, Directions direction) const
    -> std::tuple<int, int> {
  switch (direction) {
  case Directions::UP:
    row -= 1;
...
auto Labyrinth::movePlayer2D(int oldRow, int oldCol, int row, int col)
